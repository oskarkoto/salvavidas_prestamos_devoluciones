...
class PrestamoSuministro {
    ...
    **/
    ...
    function selectLast() {
        $query = "SELECT idPrestamoSuministro FROM prestamosuministro ORDER BY idPrestamoSuministro DESC LIMIT 1";
        $pdo = new Connection();
        $pdo = $pdo->open();
        $result = $pdo->query($query);
        $rows = [];
        foreach ($result->fetchAll() as $row) {
            $rows[] = new PrestamoSuministro($row['idPrestamoSuministro']);
        }
        return $rows;
    }
}